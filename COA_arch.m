function [GlobalParams,GlobalMin] = COA(lu, nfevalMAX,n_packs,n_coy, initial_centers, data, ncluster, na)
%% ------------------------------------------------------------------------
...
% [GlobalParams,GlobalMin] = COA(FOBJ, lu, nfevalMAX,Np,Nc, data,
% nclusters, na);
%
...
if initial_centers == 0
    ...
    coyotes = repmat(  initial_centers, pop_total, 1);
end
...
%     costs(c,1) = FOBJ(coyotes(c,:));
    costs(c,1) = FOBJ( data, coyotes(c,:), ncluster, na);
end
nfeval = pop_total;
%% Output variables
...
%             new_cost = FOBJ(new_coyotes(c,:));
            new_cost = FOBJ( data, new_coyotes(c,:), ncluster, na);
            nfeval   = nfeval+1;
...
%         pup_cost    = FOBJ(pup);
        pup_cost    = FOBJ( data, pup, ncluster, na);
